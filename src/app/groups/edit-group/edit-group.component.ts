...
export class EditGroupComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.editMode) {
      ...
      this.groupsService.updateGroup({ group: data }).subscribe(() => {
        this.goBack();
        this.groupsService.groupsChanged.next(this.groups);
      });
    } else {
      console.log("add mode");
      this.groupsService.addGroup({ group: data }).subscribe(group => {
  ...
  private initForm() {
    ...
    if (this.editMode) {
      const group = this.groupsService.getGroup(this.id).subscribe(group => {
